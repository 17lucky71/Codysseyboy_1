...
#### 1.1 시나리오 생성
Make.com 대시보드 → [+ Create] 버튼
시나리오 이름: "Email Analysis Pipeline"
[Create Scenario]
...
Gmail 모듈의 우측 [+] 클릭
"OpenAI" 앱 검색
액션: "Create a Completion"
API Key: [OpenAI 키 입력]
Model: gpt-4o
Temperature: 0.3
Max Tokens: 500
System Prompt: [아래 텍스트 사용]
User Input: {{1.subject}} + {{1.text_plain}}
[Save]
...
type (문의유형): 기술지원 / 결제문제 / 제품문의 / 계약관련 / 기타
summary (3줄 요약): 핵심만 3문장 이내로 간결하게
urgency (긴급도):
...
Route A의 Google Sheets 모듈 우측 [+]
"Discord" 앱 검색
액션: "Send a Message"
Webhook URL: [Discord 웹훅 URL 입력]
메시지 포맷 (아래 참고)
[Save]
...
Zapier.com → [Create] 버튼
Trigger App: "Gmail" 선택
...
[+ Add Action] 클릭
"OpenAI" 앱 검색
액션: "Create Completion"
API Key: [OpenAI 키]
Model: gpt-4o
Prompt: {{subject}} + {{plain_text_body}}
System Prompt: [위의 Make와 동일한 프롬프트 사용]
[Continue]
...
[+ Add Action] 클릭
"Google Sheets" 앱
액션: "Create Spreadsheet Row"
Google 계정: [연동]
Spreadsheet: [생성한 Sheets 선택]
Worksheet: "Emails"
Column 매핑:
A: {{timestamp}}
B: {{from_email}}
...
[+ Add Action] 클릭
"Discord" 앱
액션: "Send Channel Message"
Webhook URL: [Discord 웹훅]
메시지:
🚨 긴급 이메일 수신!
발신자: {{from_email}}
제목: {{subject}}
...
#### 2.1 Zap 활성화
Zapier 대시보드에서 생성한 Zap 선택
상단 [On/Off] 토글 → ON
"Last checked: just now" 확인
